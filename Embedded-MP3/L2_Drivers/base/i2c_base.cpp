...
bool I2C_Base::initSlave(uint8_t slaveAddr, volatile uint8_t * buffer, uint32_t size)
{
    //I2EN set to 1
//    LPC_I2C2->I2CONSET &= ~(0b11111 << 2);
    LPC_I2C2->I2CONSET = (0b10001 << 2);
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    enum {
        ...
        we_got_addressed_ack_was_sent = 0x60,

        byte_was_captured_ack_was_sent = 0x80,

        slaveArbitrationlost = 0x68,
        genAddrReceivedAckReturn = 0x70,
        slaveArbitLostGenReceive = 0x78,
        slaveAddrDataReceivedNackReturn = 0x88,
        genCallDataReceivedAckReturn = 0x90,
        genAddrDataReceivedNackReturned = 0x98,
        slaveStopStartRepeat = 0xA0,

        // Slave Transmitter States :
        slaveAddrReadAckReturn = 0xA8,
        slaveArbitrationLostslaveCalled = 0xB0,
        slaveAddrDataSentAckRecieved = 0xB8,
        slaveAddrDataSentNackRecieved = 0xC0,
        lastByteSentAckRecieved = 0xC8
...
//    printf("State = 0x%02x\n", mpI2CRegs->I2STAT);
    ...
    {
    case slaveStopStartRepeat:
            setAckFlag();
            clearSIFlag();
        break;
    case slaveAddrReadAckReturn:
        if(reg_num > buffer_size)
        {
//            printf("Buffer Reset to 0 %d.\n", reg_num);
            reg_num = 0;
        }
        ...
        clearSIFlag();
        break;
    case slaveAddrDataSentAckRecieved:
        if(reg_num > buffer_size)
        {
//            printf("Buffer Reset to 0 %d.\n", reg_num);
            reg_num = 0;
        ...
        clearSIFlag();
        break;
    case slaveAddrDataSentNackRecieved:
        setAckFlag();
        clearSIFlag();
        break;
    case we_got_addressed_ack_was_sent:
        reg_num = 0;
        is_reg_byte = true;
        setAckFlag();
        clearSIFlag();
    ...
    // 1st byte = reg num
    // 2nd byte = real data
    case byte_was_captured_ack_was_sent:
        if (is_reg_byte) {
            is_reg_byte = false;
            reg_num = mpI2CRegs->I2DAT;
//            printf("Reg: %2x\n", reg_num);
        }
        else
        {
            buffer_location[reg_num++] = mpI2CRegs->I2DAT;
        }
        setAckFlag();
        clearSIFlag();
        break;
